    ...
    fillCustomerInfo(createAccount : boolean = true) {
        const email = faker.internet.email();
            if ( DuckUtils.getRelativeUrl(browser) === '/create_account'){
                ...
                customerRegistration.$('input[name="firstname"]').setValue(faker.name.firstName());
                customerRegistration.$('input[name="lastname"]').setValue(faker.name.lastName());
                customerRegistration.$('select[name="country_code"]').selectByVisibleText('Ukraine');
                customerRegistration.$('input[name="email"]').waitForDisplayed();
                customerRegistration.$('input[name="email"]').setValue(email);
                customerRegistration.$('input[name="phone"]').setValue(faker.phone.phoneNumber('+380#########'));
                ...
                customerCheckout.$('input[name="firstname"]').setValue(faker.name.firstName());
                customerCheckout.$('input[name="lastname"]').setValue(faker.name.lastName());
                customerCheckout.$('select[name="country_code"]').selectByVisibleText('Ukraine');
                customerCheckout.$('input[name="email"]').waitForDisplayed();
                customerCheckout.$('input[name="email"]').setValue(email);
                customerCheckout.$('input[name="phone"]').setValue(faker.phone.phoneNumber('+380#########'));
                customerCheckout.$('input[name="address1"]').setValue(faker.address.streetAddress());
            ...
            throw new Error(`should be called only on a registration or checkout pages! `);
        }
    }
